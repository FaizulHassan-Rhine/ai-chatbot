...
export async function deleteChat(chatId) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Delete messages in a separate transaction
      if (messages.length > 0) {
        const messagesTransaction = db.transaction([STORES.messages], 'readwrite');
        ...
        messages.forEach((message) => {
          ...
          deleteRequest.onsuccess = () => {
            messagesDeleted++;
            if (messagesDeleted === messages.length) {
              // All messages deleted, now delete the chat
              deleteChatFromStore(db, chatId, resolve, reject);
            }
          };
          deleteRequest.onerror = () => {
            console.error('Error deleting message:', deleteRequest.error);
            reject(deleteRequest.error);
          };
        });
        
        messagesTransaction.onerror = () => {
          console.error('Transaction error deleting messages:', messagesTransaction.error);
          reject(messagesTransaction.error);
        };
      } else {
...
function deleteChatFromStore(db, chatId, resolve, reject) {
  const chatTransaction = db.transaction([STORES.chats], 'readwrite');
  const chatsStore = chatTransaction.objectStore(STORES.chats);
  ...
  chatTransaction.onerror = () => {
    console.error('Transaction error deleting chat:', chatTransaction.error);
    reject(chatTransaction.error);
  };
}
